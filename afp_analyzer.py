#!/usr/bin/env python3
"""
AFP File Analyzer - Analyzes AFP files and provides detailed information.
"""
...
def print_analysis_report(analysis: Dict[str, Any]) -> None:
    """
    Print a human-readable report of the AFP file analysis.
    
    Args:
        analysis: Analysis results from analyze_afp_file
    """
    print("\n" + "=" * 60)
    print(f"AFP File Analysis Report: {analysis['file_path']}")
    print("=" * 60)
    ...
    if analysis["fields"]:
        ...
        for i, field in enumerate(analysis["fields"][:10]):
            print(f"  {i}: offset={field['offset']}, length={field['length']}, type={field['type_code']} ({field['type_name']})")
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print(f"Usage: {sys.argv[0]} <afp_file> [--verbose]")
        sys.exit(1)
    
    file_path = sys.argv[1]
    verbose = "--verbose" in sys.argv
    
    analysis = analyze_afp_file(file_path, verbose)
